...
main = do
    ...
    let gals = getGalaxies $ getIndices $ lines input 
    putStrLn $ show $ (sumDiffs $ fst gals) + 
                      (sumDiffs $ snd gals)

getGalaxies :: [(Char, Integer, Integer)] -> ([Integer], [Integer])
getGalaxies = (\poss -> ((map fst poss),(sort $ map snd poss))) .
              map (\(_,i,j) -> (i,j)) . 
              filter (\(x,_,_) -> x == '#')

getIndices :: [[Char]] -> [(Char, Integer, Integer)]
getIndices css = concat $
                 [[(x,i,j) | (x, j) <- zip cs (getRowIndices 0 $ transpose css)] | 
                  (cs, i) <- zip css (getRowIndices 0 css)] 

getRowIndices :: Integer -> [[Char]] -> [Integer] 
getRowIndices _ []  = []
getRowIndices i (cs:css) | all (=='.') cs = i:(getRowIndices (i+1000000) css) 
