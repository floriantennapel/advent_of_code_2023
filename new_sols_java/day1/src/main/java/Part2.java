...
public class Part2 {
  ...
  /**
   * @param line
   * @param first look for the first or last number
   * @return the first or last number on the line, -1 if none are found
   */
  private static int getFirstOrLastNum(String line, boolean first) throws RuntimeException {
    ...
    for (int i = first ? 0 : chars.length - 1;
      ...
      for (int j = i; j <= i + 5 && j < chars.length; j++) { // longest number-word is 5 characters
